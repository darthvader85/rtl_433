...
/*
 * TFA Dostmann - Rain Gauge 30.3161
 *
 * Copyright (C) 2017 Stefan Klammer
...
static int tfa_rain_gauge_303161_process_row(int row, const bitbuffer_t *bitbuffer)
{
  data_t *data;
  ...
  float tempC = (negative_sign ? -( (1<<9) - temp ) : temp ) * 0.1F;
  {
    ...
    data = data_make("time",          "",            DATA_STRING, time_str,
                     "model",         "",            DATA_STRING, "TFA Monsun 30.3161",
...
static int tfa_rain_gauge_303161_callback(bitbuffer_t *bitbuffer)
{
  int counter = 0;
  for(int row=0; row<bitbuffer->num_rows; row++)
    counter += tfa_rain_gauge_303161_process_row(row, bitbuffer);
  return counter;
}
...
int main()
{
  ...
  for(size_t i=0; i<sizeof(b)/sizeof(*b); i++)
  {
    ...
    tests += tfa_rain_gauge_303161_callback(&bb);
  }
...
r_device tfa_rain_gauge_303161 = {
  .name          = "TFA Monsung 30.3161",
  .modulation    = OOK_PULSE_PPM_RAW,
  .short_limit   = 2800,
  .long_limit    = 8000,
  .reset_limit   = 8000,
  .json_callback = &tfa_rain_gauge_303161_callback,
  .disabled      = 0,
  .demod_arg     = 0,
